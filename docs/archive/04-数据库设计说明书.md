...
开发、测试、生产环境禁止依赖 Hibernate `ddl-auto=update` 自动改表。项目已引入 **Flyway** 管理迁移脚本，`spring.jpa.hibernate.ddl-auto=none`。迁移脚本路径为 `book-management-backend/src/main/resources/db/migration/`。
...
CREATE TABLE sys_user (
    ...
    openid VARCHAR(100),              -- 微信登录 openid（V3 迁移脚本追加）
...
CREATE UNIQUE INDEX uk_sys_user_openid ON sys_user(openid) WHERE openid IS NOT NULL AND deleted = FALSE;
...
当前项目已使用 Flyway 管理迁移，脚本位于 `book-management-backend/src/main/resources/db/migration/`，现有脚本包括：

| 脚本 | 说明 |
|------|------|
| `V1__init_schema.sql` | 初始化全部表结构、索引、视图 |
| `V2__init_data.sql` | 初始化基础数据（角色、权限、管理员等） |
| `V3__add_openid_to_sys_user.sql` | 为 `sys_user` 增加 `openid` 字段，支持微信登录 |
| `V4__fix_default_passwords.sql` | 修正默认用户密码哈希 |

开发约束：
1. 每次新增字段、索引、枚举值必须新增迁移脚本，不允许直接修改已发布脚本。
2. 后端 Entity 字段名、DTO 字段名、接口文档字段名必须与本数据库文档保持可追溯关系。
3. 删除业务数据默认采用逻辑删除；物理删除仅允许用于中间表或明确无审计价值的数据。
4. 库存扣减必须使用数据库条件更新或乐观锁，禁止先查库存再无保护地更新库存。
5. 测试数据应覆盖管理员、图书管理员、普通读者、无库存图书、逾期借阅、预约队列等场景。
